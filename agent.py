...
class ResearchAgent:
    ...
    def research_topic(self, topic, max_sources=3, max_queries=3, status_callback=None):
        ...
        # Log the exchange into conversation history so follow-up
        # questions (which DO go through the tool-calling agent loop)
        # have this context available.
...
"""
    ...
    def chat(self, user_message, status_callback=None):
        ...
        # Safety net: if we hit MAX_TOOL_ITERATIONS without a final
        # answer, stop instead of looping forever, and say so honestly
        # (this is the reliability mechanism required by the brief).
        return (
    ...
    def _execute_tool(self, name, args):
        ...
        try:
            ...
            if name == "summarize_source":
                # Reliability note: we deliberately do NOT trust the
                # model to relay the full scraped text back verbatim
                # in `args["content"]` (small/local models often
                # truncate or paraphrase long arguments). Instead we
                # use the actual scraped content we already stored.
                ...
                # RAG: index the full scraped content (not just the
                # summary) so later factual questions can retrieve
                # detail that a short summary might have dropped.
                try:
                    self.vector_store.add_source(title, url, content)
                except Exception:
                    pass  # indexing failure shouldn't break the research flow
            ...
            if name == "generate_report":
                # Uses whatever sources this session has collected so far,
                # not anything the model tries to pass in as arguments.
                if not self.sources:
            ...
            # Reliability mechanism: never let a tool crash the agent loop.
...
# ------------------------------------------------------------------
# Simple manual test (requires a running local Ollama with the model
# in tools.OLLAMA_MODEL already pulled: `ollama pull qwen2.5:7b`)
# ------------------------------------------------------------------
